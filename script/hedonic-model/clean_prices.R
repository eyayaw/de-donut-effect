...
home_purchases = fread("data/home-purchases_2007-2021.csv")
apart_purchases = fread("data/apartment-purchases_2007-2021.csv")

# rm duplicates
keys = c("obid", "year", "ad_begin_mon")

## homes
nh = nrow(home_purchases)
# set key for reordering the data.table in RAM by reference
setkeyv(home_purchases, c(keys, "spell")) # spell is chronologically ordered
# get all (likely) duplicates with `dupID_gen == 1`
idx = home_purchases[dupID_gen == 1, which = TRUE]
# keep the latest ad for the likely duplicates
dups = home_purchases[idx, ]
dups = dups[dups[, .(idx = .I[which.max(spell)]), .(obid, year)]$idx,]

home_purchases = rbind(home_purchases[-idx, ], dups, use.names = TRUE)
message(sprintf("%.2f%% were duplicates.", 100-100*nrow(home_purchases)/nh))
rm(dups, idx)

## apartments
na = nrow(apart_purchases)
setkeyv(apart_purchases, c(keys, "spell"))
idx = apart_purchases[dupID_gen == 1, which = TRUE]
dups = apart_purchases[idx, ]
dups = dups[dups[, .(idx = .I[which.max(spell)]), .(obid, year)]$idx,]

apart_purchases = rbind(apart_purchases[-idx, ], dups, use.names=TRUE)
message(sprintf("%.2f%% were duplicates.", 100-100*nrow(apart_purchases)/na))
rm(idx, dups)


# how to combine houses and flats?
# assign a special value for vars that do not apply to one or another (flats/homes)
not4Homes = setdiff(names(apart_purchases), names(home_purchases))
not4Aparts = setdiff(names(home_purchases), names(apart_purchases))
special_code = 999L # by construction

home_purchases[, (not4Homes) := special_code]
apart_purchases[, (not4Aparts) := special_code]
purchases = rbindlist(list(home_purchases, apart_purchases), use.names=TRUE)
...
order_of_vars = c(
  ...
  "ad_begin_mon", "mon", "price", "floor_space", "usable_floor_space", "plot_size",
...
rm(home_purchases, apart_purchases)
...
purchases = merge(purchases, districts, by.x = "did", by.y = "ags")
rm(districts)
...
# )
# cbds = st_read("data/geodata/labor-market-regions/Labor-Market-Regions_Kosfeld-Werner-2012_2019.gpkg")[, c("amr_id", "geom")] |>
#   st_centroid(cbds)
...
setkeyv(purchases, c("did", "zipcode", "mon", "year"))
...
# (ii) floor space below 30 m2 or above 500 m2
n = nrow(purchases)
purchases = purchases[
  exp(lnprice_sqm) >= 250 & exp(lnprice_sqm) <= 25000 & floor_space >= 30 & floor_space <= 500,
  ]
1-nrow(purchases)/n # duplicates

# write to disk ----
if (!dir.exists("data/processed/")) {
  dir.create("data/processed")
}
fpath = 'data/processed/purchases_clean.csv' # path for saving the file
